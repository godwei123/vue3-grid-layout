...
const resizableHandleClass = computed<string>(() =>
  renderRtl.value ? 'vue3-resizable-handle vue3-rtl-resizable-handle' : 'vue3-resizable-handle'
)
    ...
    case 'dragmove': {
      ...
      if (state.bounded) {
        const bottomBoundary =
          (event.target as HTMLElement).offsetParent!.clientHeight -
...
watch(
  ...
  (value: [number, number]) => {
    if (
      !value ||
      (Number(value[0]) === Number(state.margin[0]) && Number(value[1]) === Number(state.margin[1]))
    ) {
      return
    }
